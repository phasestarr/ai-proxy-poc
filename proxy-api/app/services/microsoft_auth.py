"""
Purpose:
- Implement the backend-driven Microsoft OAuth flow for this BFF.

Responsibilities:
- Start Microsoft authorization-code login redirects
- Persist short-lived OAuth transaction state in PostgreSQL
- Complete the callback and issue a local browser session

Notes:
- The browser still only carries the local HttpOnly `session_id` cookie.
- This module intentionally stops at local session issuance.
"""
...
from app.db.postgres.models.auth import AuthIdentity, OAuthTransaction
from app.db.postgres.models.user import User
from app.services.auth import CreatedSession, issue_session
from app.services.auth_security import decrypt_auth_payload, encrypt_auth_payload
...
def complete_microsoft_authorization(
    ...
    *,
    ...
    return created_session, return_to
    ...
    *,
    ...
    identity = db.execute(
        select(AuthIdentity).where(
            AuthIdentity.provider == MICROSOFT_PROVIDER,
