...
- `root-proxy` overwrites `X-Real-IP` and all `X-Forwarded-*` headers before proxying to `ai-proxy`
...
- Do not publish `frontend` or `backend` directly on a public host unless the replacement edge also sanitizes forwarded headers
