...
2. provider `file_id` is the real remote identity key
3. `filename` is only a human label
4. cleanup by filename is broad and should only be used for unique test names

This matters for pasted screenshots because browsers often hand you a generic name such as `image.png`.
That name is fine for normal app behavior because the database and providers use ids internally, but it is a bad cleanup selector if many files share it.
...
```powershell
cd deploy
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files list-db
```

List DB-tracked provider refs:

```powershell
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files list-db
```

Filter DB refs by provider:

```powershell
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files list-db --provider openai
```

List remote OpenAI files:

```powershell
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files list-openai --limit 100
```

List remote Anthropic files:

```powershell
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files list-anthropic --limit 100
```

Filter remote lists by exact filename:

```powershell
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files list-openai --filename test.pdf
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files list-anthropic --filename test.pdf
```

### Filename cleanup warning

This command deletes every matching remote file with that exact filename and clears matching DB refs:

```powershell
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files cleanup-filename --filename test.pdf
```

Do not aim this at generic names such as `image.png` unless you really intend to wipe all matching provider files for that provider set.

Provider-specific cleanup:

```powershell
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files cleanup-filename --provider openai --filename test.pdf
```

Remote-only cleanup without DB changes:

```powershell
docker compose --env-file ../.env.local -f docker-compose.yml -f docker-compose.local.yml exec -T proxy-api python -m app.maintenance.attachment_provider_files cleanup-filename --filename test.pdf --skip-db-clear
```
...
Use direct provider APIs when you need to delete one exact `file_id`.

OpenAI list:

```powershell
curl.exe -sS https://api.openai.com/v1/files?purpose=user_data&order=desc&limit=100 -H "Authorization: Bearer $env:OPENAI_API_KEY"
curl.exe -sS https://api.openai.com/v1/files?purpose=vision&order=desc&limit=100 -H "Authorization: Bearer $env:OPENAI_API_KEY"
```

OpenAI delete one exact file id:

```powershell
curl.exe -sS -X DELETE https://api.openai.com/v1/files/PUT_OPENAI_FILE_ID_HERE -H "Authorization: Bearer $env:OPENAI_API_KEY"
```

Anthropic list:

```powershell
curl.exe -sS "https://api.anthropic.com/v1/files?limit=100" -H "x-api-key: $env:ANTHROPIC_API_KEY" -H "anthropic-version: 2023-06-01" -H "anthropic-beta: files-api-2025-04-14"
```

Anthropic delete one exact file id:

```powershell
curl.exe -sS -X DELETE "https://api.anthropic.com/v1/files/PUT_ANTHROPIC_FILE_ID_HERE" -H "x-api-key: $env:ANTHROPIC_API_KEY" -H "anthropic-version: 2023-06-01" -H "anthropic-beta: files-api-2025-04-14"
```
