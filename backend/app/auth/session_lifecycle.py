...
from app.auth.session_policy import get_session_limit, get_session_ttl
...
def issue_session(
    ...
    *,
    ...
    for expired_session in expired_sessions:
        expired_session.superseded_by_session_id = auth_session.id
    ...
    *,
    ...
    if auth_session.state != "active":
        return SessionLookupResult(
            ...
            should_clear_cookie=auth_session.expired_reason_code != "evicted_by_session_limit",
            auth_type=auth_session.auth_type,
            session_limit=get_session_limit(auth_session.auth_type),
            can_evict_oldest=auth_session.expired_reason_code == "evicted_by_session_limit",
        )
    ...
    if touch:
        ...
        auth_session.expires_at = now + get_session_ttl()
        user.last_seen_at = now
        db.commit()
...
def _enforce_session_limit(
    ...
    *,
    ...
    sessions_to_expire = active_sessions[:overflow]
    for auth_session in sessions_to_expire:
        expire_session(
            auth_session,
            now=now,
            reason_code="evicted_by_session_limit",
            reason=(
                f"Expired to make room for a newer {auth_type} session "
...
def expire_session(
    ...
    *,
    ...
    auth_session.expired_at = now
    auth_session.expired_reason_code = reason_code
    auth_session.expired_reason = reason
...
def is_session_expired(
    ...
    *,
    ...
    reference_time = auth_session.last_seen_at or auth_session.created_at
    return reference_time + get_session_ttl() <= now
